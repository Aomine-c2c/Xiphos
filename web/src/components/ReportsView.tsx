...
export default function ReportsView() {
  ...
  const kpis = [
    { label: "PROFIT FACTOR", value: performanceMetrics?.profit_factor.toFixed(2) || "0.00", sub: "EFFICIENCY RATING", colorClass: "text-xiphos-green" },
    { label: "WIN RATE", value: `${performanceMetrics?.win_rate.toFixed(1) || 0}%`, sub: "RELIABILITY", colorClass: "text-xiphos-blue" },
    { label: "MAX DRAWDOWN", value: `-$${performanceMetrics?.max_drawdown.toFixed(2) || 0}`, sub: "PEAK TO TROUGH", colorClass: "text-xiphos-red" },
    { label: "TOTAL RETURN", value: `+$${performanceMetrics?.total_profit.toFixed(2) || 0}`, sub: "SINCE INCEPTION", colorClass: "text-xiphos-green" },
  ];
  ...
  const drawChart = () => {
    ...
    return (
      <svg width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} className="overflow-visible">
        ...
        <circle cx={eC.at(-1)!.x} cy={eC.at(-1)!.y} r="4" fill="#00D26A" />
  ...
  return (
    <div className="flex flex-col w-full h-full font-mono select-none overflow-hidden gap-2">
      <div className="bg-xiphos-panel border border-slate-900/80 rounded-sm flex flex-col overflow-hidden flex-1 min-h-0">

        {/* Header */}
        <div className="p-3.5 border-b border-slate-950 flex items-center bg-xiphos-bg/40 shrink-0">
        ...
        <div className="flex-1 min-h-0 grid grid-cols-12 overflow-hidden">
          ...
          <div className="col-span-3 border-r border-slate-900/60 p-4 flex flex-col gap-3 overflow-hidden">
            ...
            {kpis.map(kpi => (
              <div key={kpi.label} className="bg-xiphos-bg/40 border border-slate-900/60 rounded-sm p-3">
                <div className="text-[15px] text-[#6f7e90] font-black uppercase tracking-wider mb-1">{kpi.label}</div>
                <div className={`text-[26px] font-black leading-none ${kpi.colorClass}`}>{kpi.value}</div>
                <div className={`text-[14px] mt-0.5 ${kpi.colorClass}`}>{kpi.sub}</div>
              </div>
            ))}

            <div className="border-t border-slate-950 pt-3 mt-auto">
              <div className="space-y-1.5 text-[15px] text-[#8e9aa8]">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-xiphos-green font-bold">SHIELD v2</span>
                </div>
                <div className="flex justify-between">
          ...
          <div className="col-span-9 p-4 flex flex-col gap-4 overflow-hidden">
            ...
            {/* Equity curve */}
            <div className="bg-xiphos-bg/40 border border-slate-900/60 rounded-sm p-3.5 shrink-0 h-[165px]">
              <div className="flex items-center justify-between mb-2">
                <span className="text-[15px] text-[#6f7e90] font-black uppercase tracking-wider">HISTORICAL EQUITY GROWTH CURVE</span>
                <span className="text-[17px] font-black text-xiphos-green">CURRENT: ${account.equity.toFixed(2)} USD</span>
              </div>
            ...
            <div className="flex-1 min-h-0 flex flex-col">
              <span className="text-[15px] text-[#6f7e90] font-black uppercase tracking-wider block mb-2.5 border-b border-slate-950 pb-1.5 shrink-0">
                RECENT COMPLIANCE & PERFORMANCE EXPORTS
              </span>
              <div className="space-y-2 flex-1 min-h-0 overflow-hidden">
                {reportsList.map(report => (
                  <div key={report.id}
                    className="border border-slate-950 bg-xiphos-panel/60 hover:bg-xiphos-panel transition-all p-3 rounded-sm flex items-center justify-between">
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-2">
                        <span className="px-1.5 py-0.5 border border-xiphos-blue/40 text-xiphos-blue bg-xiphos-blue/5 rounded-sm font-black text-[14px] uppercase">
                          {report.type}
                        </span>
                        <span className="text-white font-bold text-[16px]">{report.name}</span>
                      </div>
                      <div className="text-[15px] text-[#6f7e90]">
                    ...
                    <button title="Download Report"
                      ...
                      className="p-1.5 bg-slate-950 hover:bg-xiphos-blue/20 text-[#6f7e90] hover:text-xiphos-blue border border-slate-900 hover:border-xiphos-blue/40 rounded-sm cursor-pointer transition-all">
      ...
      <div className="bg-xiphos-panel border border-slate-900/80 rounded-sm p-3 shrink-0">
        <div className="flex items-center justify-between text-[16px] font-black">
          ...
          <span className="text-[#8e9aa8] text-[15px]">
            PROFIT FACTOR: <span className="text-xiphos-green font-black">{performanceMetrics?.profit_factor.toFixed(2) || "0.00"} (HIGH EFFICIENCY)</span>
          </span>
        </div>
      </div>
    </div>
