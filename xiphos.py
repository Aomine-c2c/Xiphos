...
def enqueue_output(name, out):
    for line in iter(out.readline, b''):
        ...
        if decoded:
            # Suppress noisy Next.js compilation logs
...
def main():
    ...
    # Prefer local .venv if it exists, otherwise fallback to sys.executable
        ...
        if proc.poll() is not None:
            ...
            with log_lock:
                for line in log_queue:
                    print(line)
            sys.exit(1) # atexit will automatically call kill_all()
