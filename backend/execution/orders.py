...
def _get_trade_price_and_stoplevel(symbol, type_str, sl_price):
    ...
    # MT5 Python API uses 'filling_mode' (not 'trade_filling_mode')
    filling_bitmask = getattr(info, "filling_mode", 0)

    return price, filling_bitmask
...
def _execute_order_with_modes(request, supported_modes, symbol, type_str, volume, sl_price, magic, sig=None):
    ...
    for filling_mode, filling_name in supported_modes:
        ...
        if result:
            ...
            logger.info(f"Trade successfully opened! Ticket: {result.order} (Latency: {latency_ms:.2f}ms)")
...
def open_trade(symbol: str, type_str: str, volume: float, sl_price: float, magic: int, sig: dict = None, tp: float = None):
...
def modify_sl(ticket: int, symbol: str, new_sl: float):
    ...
    if not positions:
        ...
        logger.warning(f"Attempted to modify SL for unknown ticket {ticket}")
