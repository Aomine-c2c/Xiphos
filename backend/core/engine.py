...
class XiphosEngine:
    ...
    def _evaluate_gate_2(self) -> bool:  # noqa: C901
        ...
        for name, symbols in settings.correlation_groups.items():
            ...
            if blocking:
                ...
                blocked_buckets.append(name)

        if blocked_buckets:

            self.last_cycle_data["gates"]["gate_2_correlation"] = "FAIL"

            self.last_cycle_data["gates"]["gate_2_details"] = f"{len(blocked_buckets)} BLOCKED"
        ...
        self.last_cycle_data["gates"]["gate_2_details"] = "NO BLOCK"
    ...
    def _gather_signals(self, recent_win_rate: float) -> list:
        ...
        for sym in all_symbols:
            ...
            from data_pipeline.l2_fetcher import fetch_l2_imbalance
            
            # Fetch News and Macro from Redis cache
            news_raw = redis_client.client.get("xiphos:data:news")
            macro_raw = redis_client.client.get("xiphos:data:macro")
            
            news_data = json.loads(news_raw) if news_raw else {"sentiment": "NEUTRAL", "recent_headlines": []}
            macro_data = json.loads(macro_raw) if macro_raw else {"upcoming_events": [], "volatility_flag": False}
            
            # Fetch L2 Data specifically for this symbol right now
            l2_data = fetch_l2_imbalance(sym)

            mahoraga_engine.evaluate(sym, ind_data, recent_win_rate, news_data, macro_data, l2_data)
            ...
            ind_data["tp_multiplier"] = params.tp_multiplier
            ind_data["trading_halted"] = params.trading_halted
            ind_data["active_strategy"] = params.active_strategy
            ...
            if params.trading_halted:
                # Mahoraga has forbidden trading on this asset
                continue
                
            # Strategy router
            if params.active_strategy == "TREND_FOLLOWING":
                signal = evaluate_signal(ind_data)
            elif params.active_strategy == "MEAN_REVERSION":
                # Mock mean reversion signal evaluator for now
                # In the future, this will call `strategies.mean_reversion.evaluate_signal(ind_data)`
                log.info(f"{sym} switched to MEAN_REVERSION strategy. Awaiting implementation.")
                signal = None
            else:
                signal = None
    ...
    def _execute_signal(self, sig, slots_available, open_counts, session_blocked_buckets) -> int:
        ...
        tp_multiplier = ind.get("tp_multiplier", 1.0)
        ...
        if open_count < 2 and slots_available > 0:
            ...
            # Dynamic Take Profit for scalper (distance * tp_multiplier)
            tp_scalper = round(entry_price + (sl_raw_dist_a * tp_multiplier), 5) if typ == "BUY" else round(entry_price + (sl_raw_dist_a * tp_multiplier), 5) # Note: order logic is handled in MT5 or here depending on direction. 
            # Actually, sl_raw_dist_a is positive for BUY (price - low), negative for SELL (price - high).
            tp_scalper = round(entry_price + (sl_raw_dist_a * tp_multiplier), 5)

            risk_a = mt5.order_calc_profit(order_type, sym, adapted_lot, entry_price, sl_scalper)
            if risk_a is None or abs(risk_a) <= 10.0:
                start_exec = time.time()
                res_a = open_trade(sym, typ, adapted_lot, sl_scalper, magic_a, sig=sig, tp=tp_scalper)
        ...
        if open_count < 2 and slots_available > slots_consumed:
            ...
            # Runner uses a larger TP multiplier (e.g. 3x the scalper TP)
            tp_runner = round(entry_price + (sl_raw_dist_b * tp_multiplier * 3.0), 5)

            risk_b = mt5.order_calc_profit(order_type, sym, adapted_lot, entry_price, sl_runner)
            if risk_b is None or abs(risk_b) <= 10.0:
                start_exec = time.time()
                res_b = open_trade(sym, typ, adapted_lot, sl_runner, magic_b, sig=sig, tp=tp_runner)
